"""Preflight-first M8_v1c training entrypoint with a verified repo-local runtime hook."""
...
OUTPUT_DIR = Path("artifacts/m8_v1c_training_entrypoint")
SUMMARY_PATH = OUTPUT_DIR / "m8_v1c_training_entrypoint_summary.json"
CHECK_PATH = OUTPUT_DIR / "m8_v1c_training_entrypoint_check.csv"
COMMAND_DRAFT_PATH = OUTPUT_DIR / "m8_v1c_training_command_draft.txt"
MANIFEST_PATH = OUTPUT_DIR / "m8_v1c_training_entrypoint_non_execution_manifest.json"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate or execute the repo-local M8_v1c training entrypoint."
    )
    parser.add_argument("--data", required=True, help="Dataset YAML path.")
    parser.add_argument("--model", required=True, help="Model YAML path or basename.")
    parser.add_argument("--method-config", required=True, help="M8_v1c runtime method config YAML path.")
...
def validate_method_policy(path: Path) -> tuple[bool, str]:
    ...
    expected_max = {"small": "P3", "medium": "P5", "large": "P5"}
    issues: list[str] = []
    for group_name, expected_scale in expected_max.items():
        resolved_max = max(policy.weights_for_group(group_name), key=policy.weights_for_group(group_name).get)
        if resolved_max != expected_scale:
            issues.append(f"{group_name} must peak at {expected_scale}, got {resolved_max}")
    ...
    return not issues, "; ".join(issues) if issues else "runtime M8_v1c policy verified"
...
def build_readme(summary: dict[str, Any], artifact_paths: list[str], check_rows: list[CheckRow]) -> str:
    ...
    lines = [
        ...
        "",
        "This directory contains the repo-local M8_v1c training entrypoint preflight bundle.",
        "",
        "## Summary",
