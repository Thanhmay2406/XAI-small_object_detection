"""Synthetic runtime verification for the repo-local weighting hook."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Synthetic-only runtime integration check.")
    ...
    parser.add_argument("--method-config", type=Path, required=True, help="Path to runtime weighting config YAML.")
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("artifacts/runtime_integration_check/runtime_integration_report.json"),
...
def main() -> None:
    ...
    fake_batch = {
        ...
        "bboxes": torch.tensor(
            [
                [0.25, 0.25, 0.03, 0.03],  # small object on sample 0
                [0.75, 0.75, 0.20, 0.20],  # large object on sample 0
                [0.50, 0.50, 0.10, 0.10],  # medium object on sample 1
            ],
            dtype=torch.float32,
        ),
